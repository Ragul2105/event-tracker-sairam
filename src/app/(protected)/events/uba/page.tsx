...
export default function UBAPage() {
  ...
  async function fetchEvents() {
    ...
    try {
      ...
      const unit = unitsData.data?.find((u: any) => u.code === 'UBA');
  ...
  async function handleDownloadTemplate() {
    ...
    link.href = "/uba-template.xlsx";
    link.download = "uba-template.xlsx";
  ...
  async function handleImport() {
    ...
    try {
      ...
      const res = await fetch("/api/unit-events/uba/import", {
        method: "POST",
        body: formData,
      });
      ...
      if (data.success) {
        showAlert(
          "success",
          "Import Successful!",
          `Successfully imported ${data.imported} event${data.imported !== 1 ? 's' : ''} to UBA.`
        );
